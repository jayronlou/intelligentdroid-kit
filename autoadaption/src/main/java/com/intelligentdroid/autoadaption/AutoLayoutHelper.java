...
public class AutoLayoutHelper {
        ...
        if (info != null) {
          ...
          if (params instanceof ViewGroup.MarginLayoutParams) {
            info.fillMarginLayoutParams((ViewGroup.MarginLayoutParams) params, getAvailableWidth(),
                getAvailableHeight(), getDesignWidth(), getDesignHeight());
          } else {
            info.fillLayoutParams(params, getAvailableWidth(), getAvailableHeight(),
                getDesignWidth(), getDesignHeight());
          }
        }
      }
    }
  ...
  public static class AutoLayoutInfo {
    ...
    public void fillLayoutParams(ViewGroup.LayoutParams params, int avaWidth, int avaHeight,
        int designWidth, int designHeight) {
      if (widthPx != 0) {
        if (AutoBase.contains(autobaseHeight, AutoBase.WIDTH)) {
          params.width = (int) (widthPx * 1.0f / designWidth * avaHeight);
        } else {
    ...
    public void fillMarginLayoutParams(ViewGroup.MarginLayoutParams params, int avaWidth,
      ...
      if (margin != 0) {
        if (AutoBase.contains(autobaseWidth, AutoBase.MARGIN)) {
          ...
          params.leftMargin =
              params.topMargin = params.rightMargin = params.bottomMargin = marginSize;
